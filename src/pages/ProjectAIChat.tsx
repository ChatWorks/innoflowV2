...
export function ProjectAIChat() {
  ...
  const { data: projectContext, isLoading: contextLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const tasksRes = deliverables.data?.length > 0 
        ? await supabase
            .from('tasks')
            .select('*')
            .in('deliverable_id', deliverables.data.map(d => d.id))
        : { data: [] };
  ...
  return (
    <>
      <SEO 
        title={`AI Chat - ${project?.name || 'Project'}`}
        description={`AI-gestuurde projectassistent voor ${project?.name}. Krijg inzichten, analyses en aanbevelingen voor je projectbeheer.`}
      />
      
      <div className="h-screen flex flex-col">
        {/* Header */}
        <header className="border-b bg-background px-4 py-3 flex items-center gap-3">
          <Button 
            variant="ghost" 
            size="sm"

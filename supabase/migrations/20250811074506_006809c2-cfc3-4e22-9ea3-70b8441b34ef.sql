...
-- Policies (drop then create to avoid IF NOT EXISTS)
DROP POLICY IF EXISTS "Users can view own meetings" ON public.project_meetings;
CREATE POLICY "Users can view own meetings"
ON public.project_meetings
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create own meetings" ON public.project_meetings;
CREATE POLICY "Users can create own meetings"
ON public.project_meetings
FOR INSERT
WITH CHECK ((auth.uid() = user_id) AND (user_id IS NOT NULL));

DROP POLICY IF EXISTS "Users can update own meetings" ON public.project_meetings;
CREATE POLICY "Users can update own meetings"
ON public.project_meetings
FOR UPDATE
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own meetings" ON public.project_meetings;
...
CREATE POLICY "Anon can access meetings via active portal"
ON public.project_meetings
FOR SELECT
USING (
  EXISTS (

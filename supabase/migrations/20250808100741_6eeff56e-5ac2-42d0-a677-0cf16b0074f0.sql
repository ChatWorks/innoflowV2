...
DO $$ BEGIN
  CREATE POLICY "moneybird_sync_state_insert_own"
  ON public.moneybird_sync_state
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "moneybird_sync_state_update_own"
  ON public.moneybird_sync_state
  FOR UPDATE
  USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "moneybird_sync_state_delete_own"
  ON public.moneybird_sync_state
  FOR DELETE
  USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_moneybird_sync_state_unique ON public.moneybird_sync_state(connection_id, resource);
...
DO $$ BEGIN
  CREATE POLICY "moneybird_agg_insert_own"
  ON public.moneybird_aggregates_daily
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "moneybird_agg_update_own"
  ON public.moneybird_aggregates_daily
  FOR UPDATE
  USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "moneybird_agg_delete_own"
  ON public.moneybird_aggregates_daily
  FOR DELETE
  USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- Unique guarantee across optional group_key using a functional unique index
CREATE UNIQUE INDEX IF NOT EXISTS idx_moneybird_agg_unique
ON public.moneybird_aggregates_daily(user_id, date, grouping, (COALESCE(group_key, '')), basis);